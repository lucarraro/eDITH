...
Scope (where do we write this?):

* The eDITH model is designed to work for river networks not containing lakes, reservoirs, or bifurcations in the downstream direction (e.g., braided or artificial channels creating loops in the river network).
* The spatial extent of the eDNA sampling must adequately cover the entire river network, in order to allow the model to make robust predictions. As a rule of thumb:
  - Ideally, there should be one sampling site each 10--20 km$^2$ of drained area.
  - The mean along-stream distance between sampling sites should be in the order of magnitude of some (tens of) kilometers. eDNA samples that are collected more than 50 km apart from each other are unlikely to contain overlapping information (see 3.1).
  - All main tributaries of a river network should be sampled. Ideally, sampling sites should be located just upstream of a confluence, so that the independent signals from the joining tributaries can be gauged. See [@Carraro2021] for further information on optimal eDNA sampling strategies.
* Something on microbes vs. vertebrates
...
* On equal conditions during a sampling campaign
* Well mixed samples
* River network discretized in reaches. the spatial resolution can be freely tuned via rivnet. The ensemble of reaches covers the entire river network. Node = reach
