eval_posterior_eDITH <- function(x, river, covariates, Z.normalize=TRUE, source.area="AG"){
  ...
  p_mat <- matrix(0,dim(sample)[1],river$AG$nNodes)
  C_mat <- matrix(0,dim(sample)[1],river$AG$nNodes)

  for (r in 1:dim(sample)[1]){
    ...
    if (x$ll.type=="norm") {
      probDetection_mat[r, ] <- 1 - pnorm(0, mean = local_expected_C, sd = sample[r,"sigma"])
    } else if (x$ll.type=="lnorm"){
      probDetection_mat[r, ] <- 1 - plnorm(0, meanlog = log(local_expected_C^2/sqrt(sample[r,"sigma"]^2 + local_expected_C^2)),
                                           sdlog = sqrt(log(sample[r,"sigma"]^2/local_expected_C^2 + 1)))
    } else if (x$ll.type=="nbinom"){
      probDetection_mat[r, ] <- 1 - pnbinom(0, size = local_expected_C/(sample[r,"omega"]-1),
                                            prob = 1/sample[r,"omega"])
    }
  }
